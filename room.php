...
<div id="page">
  ...
  <div id="body">
    ...
    <input type="button" value="Tambah" class="button fl-left" onclick="window.location='add-room.php';" />
    ...
    <table width="100%" border="0" class="datagrid">
		  ...
		  $q = $_GET['q'];
		  $index = mysql_query("SELECT * FROM room WHERE name LIKE '%$q%' OR description LIKE '%$q%' ORDER BY name ASC;");
		  while($row = mysql_fetch_assoc($index)) {
			  ?>
              <tr>
                <td align="center"><?= $row['id']; ?></td>
                <td><?= getCategoryName($row['room_category_id']); ?></td>
                <td><?= $row['name']; ?></td>
                <td><?= $row['description']; ?></td>
                <td align="right"><?= formatNumber('Rp. ',$row['price'],',-'); ?></td>
                <td align="center">
                	<a title="Edit" href="edit-room.php?id=<?= $row['id']; ?>"><img src="assets/img/edit.png" width="14" /></a> &nbsp; 
                    <a title="Hapus" href="delete.php?tbl=room&id=<?= $row['id']; ?>" onclick="return confirmDelete();"><img src="assets/img/delete.png" width="14" /></a>
                </td>
              </tr>  
              <?php
		  }
